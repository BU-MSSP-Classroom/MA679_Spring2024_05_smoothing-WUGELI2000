...
Your code:
```{r,echo=TRUE}
fit.poly = lm(`SalePrice` ~ poly(`Lot Area`, 4), data = ames_raw_2008)
summary(fit.poly)

poly.pred = predict(fit.poly, 
                    newdata = ames_raw_2009, 
                    se = TRUE)

poly.actual = ames_raw_2009$`SalePrice`
calc_loss(poly.pred$fit, poly.actual)
```

Your answer:

I used polynomial regression and I think this model maybe not a better prediction.
~~~
...
Your code:
```{r,echo=TRUE}
fit.poly = lm(SURVIVED_bin ~ poly(AGE, 4), data = Train_COVID)
summary(fit.poly)
...
poly.actual = Test_COVID$SURVIVED_bin
calc_loss(poly.pred$fit, poly.actual)
```

Your answer:

I used polynomial regression and I think this model is a better prediction.
...
model.lm <- lm(Outstate ~ Private + Top10perc + Expend, data = College.train)
summary(model.lm)

pred.lm <- predict(model.lm, newdata = College.test)
sum((pred.lm-College.test$Outstate)^2)
```
...
```{r,echo=TRUE}
set.seed(1)
model.gam <- gam(Outstate ~ Private + bs(Top10perc) + poly(Expend, 2), data = College.train)
...
Your code:
```{r,echo=TRUE}
College.pred <- predict(model.gam, newdata = College.test)
sum((College.pred - College.test$Outstate)^2)
```
...
Your answer:
Yes, since when use non-linear, square error become smaller.
